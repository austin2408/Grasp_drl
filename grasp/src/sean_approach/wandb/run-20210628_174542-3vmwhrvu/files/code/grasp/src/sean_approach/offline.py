...
class Option():
    def __init__(self):
        ...
        parser.add_argument("--iteration", type=int, default=50, help="The train iteration, default is 30") # M
        parser.add_argument("--memory_size", type=int, default=1064, help="The memory size, default is None")
    ...
    def training(self, args):
        ...
        for i in range(args.iteration):
            ...
            for j in range(len(mini_batch)):
                ...
                td_target = self.trainer.get_label_value(reward, next_color, next_depth, mini_batch[j].is_empty, pixel_index[0])
                loss_ = self.trainer.backprop(color, depth, pixel_index, int(td_target[0]), int(is_weight[j]), mini_batch_size, j==0, j==len(mini_batch)-1)
